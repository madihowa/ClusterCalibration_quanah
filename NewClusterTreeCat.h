...
// Thu Jul  8 21:11:23 2021 by ROOT version 6.14/04
// from TTree ClusterTree/ClusterTree
// found on file: topo-cluster.pool.root
//////////////////////////////////////////////////////////

#ifndef NewClusterTreeCat_h
#define NewClusterTreeCat_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   NewClusterTreeCat(TTree *tree=0);
   virtual ~NewClusterTreeCat();
...
#ifdef NewClusterTreeCat_cxx
NewClusterTreeCat::NewClusterTreeCat(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("topo-cluster.pool.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("topo-cluster.pool.root");
      }
...
NewClusterTreeCat::~NewClusterTreeCat()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t NewClusterTreeCat::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t NewClusterTreeCat::LoadTree(Long64_t entry)
{
...
void NewClusterTreeCat::Init(TTree *tree)
{
...
Bool_t NewClusterTreeCat::Notify()
{
...
void NewClusterTreeCat::Show(Long64_t entry)
{
...
Int_t NewClusterTreeCat::Cut(Long64_t entry)
{
...
#endif // #ifdef NewClusterTreeCat_cxx
